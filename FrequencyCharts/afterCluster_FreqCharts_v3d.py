...
# Define PSE folder
simulations_tag = "PSEmpi_3dFreqCharts4.0-m03d08y2023-t12h.46m.17s"  # Tag cluster job
...
dfsub = df.iloc[::10].dropna()

fig = go.Figure(go.Scatter3d(x=dfsub.He, y=dfsub.Hi, z=dfsub.p, mode="markers",
                             marker=dict(opacity=1, line_width=0, size=dfsub.roi1_Hz, color=dfsub.roi1_Hz)))
fig.update_layout(scene=dict(xaxis=dict(title="He"), yaxis=dict(title="Hi"), zaxis=dict(title="p")))
fig.show("browser")