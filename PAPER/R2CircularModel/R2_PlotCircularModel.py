...
simname = "vCC_cModel_AlexInit_dmnTrue_bnm-dt1sL10_m03d29y2023-t18h.01m.02s"
...
#####       3. R2a - Plot the evolution of the variables involved    ########
# in changing NMM parameters: ABt, TAUt, and HA; and parameters.

## PART 1: individual plot with secondary axis for BNM parameters
fig = go.Figure()
fig.add_trace(go.Scatter(x=timepoints, y=avg_weights, line=dict(width=line_width, color=c_sc), opacity=opacity, name="Wij", yaxis="y1", showlegend=False))
fig.add_trace(go.Scatter(x=timepoints, y=He, line=dict(width=line_width, color=c_he), opacity=opacity, name="He", yaxis="y2", showlegend=False))
fig.add_trace(go.Scatter(x=timepoints, y=Cie, line=dict(width=line_width, color=c_cie), opacity=opacity, name="Cie", yaxis="y3", showlegend=False))
fig.add_trace(go.Scatter(x=timepoints, y=Cee, line=dict(width=line_width, color=c_cee), opacity=opacity, name="Cee", yaxis="y4", showlegend=False))

fig.update_layout(template="plotly_white", height=290, width=470,
                  xaxis=dict(title="Time (years)"),
                  yaxis1=dict(title=r"$w_{ij}$", titlefont=dict(color=c_sc), tickfont=dict(color=c_sc)),
                  yaxis2=dict(title="$H_e$", titlefont=dict(color=c_he), tickfont=dict(color=c_he),
                              side="right", autoshift=True, overlaying="y", tickmode="sync"),

                  yaxis3=dict(title="$C_{ie}$", titlefont=dict(color=c_cie), tickfont=dict(color=c_cie),
                              anchor="free", overlaying="y", side="right", autoshift=True, tickmode="sync", shift=10),

                  yaxis4=dict(title="$C_{ee}$", titlefont=dict(color=c_cee), tickfont=dict(color=c_cee),
                              anchor="free", overlaying="y", side="right", autoshift=True, tickmode="sync", shift=10))

pio.write_html(fig,  main_folder + simname + "\\R2a_Part1_CircularModel_underneath.html", auto_open=True, include_mathjax="cdn")
pio.write_image(fig,  main_folder + simname + "\\R2a_Part1_CircularModel_underneath.svg")

## PART 2: Circular Plot with proteinopathy, damages and firing rate.
fig = make_subplots(rows=3, cols=3, horizontal_spacing=0.15, vertical_spacing=0.15,
                    specs=[[{},{},{}], [{},{},{}], [{},{},{}]])
# Add ABt and TAUt
fig.add_trace(go.Scatter(x=timepoints, y=ABt, name="ABt", line=dict(color=c_ab, width=line_width), opacity=opacity), row=1, col=2)
fig.add_trace(go.Scatter(x=timepoints, y=TAUt, name="TAUt", line=dict(color=c_tau, width=line_width), opacity=opacity), row=1, col=2)
# Add damages Proteinopathy
fig.add_trace(go.Scatter(x=timepoints, y=ABdam, name="ABdam", line=dict(color=c_ab, width=line_width-1),
                         opacity=opacity, showlegend=False), row=2, col=3)
fig.add_trace(go.Scatter(x=timepoints, y=TAUdam, name="TAUdam", line=dict(color=c_tau, width=line_width-1),
                         opacity=opacity, showlegend=False), row=2, col=3)
# Add damages Activity
fig.add_trace(go.Scatter(x=timepoints, y=HAdam, name="HAdam", line=dict(color=c_fr, width=line_width-1), opacity=opacity,
                         showlegend=False), row=2, col=1)
# Add firing rate
fig.add_trace(go.Scatter(x=timepoints_short, y=avg_rate, name="Firing rate", line=dict(color=c_fr, width=line_width),
                         opacity=opacity, showlegend=False), row=3, col=2)

fig.update_layout(template="plotly_white", height=600, width=900, legend=dict(x=0.8, y=0.925),
                    xaxis2=dict(title="Time (years)"),
                    xaxis4=dict(title="Time (years)"),
                    xaxis6=dict(title="Time (years)"),
                    xaxis8=dict(title="Time (years)"),
                  yaxis2=dict(title="Concentration (M)"), yaxis4=dict(title="Hyperactivity<br>damage"),
                  yaxis6=dict(title="Proteinopathy<br>damage"), yaxis8=dict(title="Firing rate (kHz)") )

pio.write_html(fig, main_folder + simname + "\\R2a_Part2_CircularModel_underneath.html", auto_open=True)
pio.write_image(fig, main_folder + simname + "\\R2a_Part2_CircularModel_underneath.svg")
