...
from ADpg_functions import circApproach, CircularADpgModel_vH, paramtraj_in3D
...
ABt_initMap = [0.005 / len(AB_seeds) if roi in AB_seeds else 0 for roi in conn.region_labels]
TAUt_initMap = [0.0005 / len(TAU_seeds) if roi in TAU_seeds else 0 for roi in conn.region_labels]
...
prop_simL, prop_dt = 30, 0.25
bnm_simL, transient, bnm_dt = 10, 2, 1  # Units (seconds, seconds, years)

circmodel = CircularADpgModel_vH(
    conn, AB_initMap, TAU_initMap, ABt_initMap, TAUt_initMap, AB_initdam, TAU_initdam, HA_initdam,
    TAU_dam2SC=5e-2, maxTAU2SCdam=0.3, HA_damrate=0.01, maxHAdam=1.25,  # origins @ ¿?, 0.01, 1.5
    init_He=3.25, init_Hi=22,   # origins 3.25, 22 || Initial values for NMM variable parameters
    rho=100, toxicSynergy=0.4,  # origins 5, 2 || rho as a diffusion constant
    prodAB=6, clearAB=6, transAB2t=6, clearABt=4.8,
    prodTAU=6, clearTAU=6, transTAU2t=6, clearTAUt=4.8)
...
param_info = "He"+str(circmodel.init_He["value"][0])+str(rHe)+ "; Hi" + str(circmodel.init_Hi["value"][0]) + str(rHi) +"; maxTAU2SC" + str(circmodel.maxTAU2SCdam["value"])
paramtraj_in3D(out_circ, "freq", "PSEmpi_3dFreqCharts4.0-m03d08y2023-t12h.46m.17s", folder=spec_fold, auto_open=True, param_info=param_info)
paramtraj_in3D(out_circ, "rate", "PSEmpi_3dFreqCharts4.0-m03d08y2023-t12h.46m.17s", folder=spec_fold, auto_open=True, param_info=param_info)
...
# np.average(out_circ[2][152][3] / out_circ[2][0][3])
