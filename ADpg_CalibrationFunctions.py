...
def simulations(params, conn, out="fft", mode="classic", rois="bnm"):
    ...
    if rois == "pair":
        ...
        if "classic" in mode:
            ...
            sigma_array = [0.15 if 'Thal' in roi else 0 for roi in conn.region_labels]
...
def TransferOne(out_prop, out_simpleSim=None, out_networkSim=None, skip=1, mode="classic", folder="figures", auto_open=True):
    ...
    if "circ" in mode:
        ...
        fig = make_subplots(rows=2, cols=3, specs=[[{"secondary_y": True}, {}, {}], [{"secondary_y": True}, {}, {}]],
                            ...
                            column_widths=[0.45, 0.275, 0.275], row_titles=["Power", "Frequency"], shared_xaxes=True,
        ...
        fig.add_trace(go.Heatmap(z=Hchart_df.roi1_auc, x=Hchart_df.He, y=Hchart_df.Hi, coloraxis="coloraxis1"), row=1,
                      col=2)
        fig.add_trace(go.Heatmap(z=Hchart_df.roi1_Hz, x=Hchart_df.He, y=Hchart_df.Hi, coloraxis="coloraxis2"), row=2,
                      col=2)

        # Taue-Taui
        tauchart_df = df.loc[(df["mode"] == "classical" + "&fixed") & (df["exp"] == "exp_tau")]
        fig.add_trace(
            go.Heatmap(z=tauchart_df.roi1_auc, x=tauchart_df.taue, y=tauchart_df.taui, coloraxis="coloraxis1"), row=1,
            col=3)
        ...
        hovertext = ["   <b>t%0.2f</b><br>He = %0.2f  |  Hi = %0.2f<br>Power (dB) %0.4f"
                     % (timepoints[ii], He[ii], Hi[ii], Hchart_df["roi1_auc"].iloc[
        ...
        if out_simpleSim:
            ...
            fig.add_trace(go.Scatter(x=timepoints[::skip], y=ss_pow, name="power_SimpleSim", legendgroup="ss",
                                     line=dict(width=4, color="gray")), row=2, col=1)
            fig.add_trace(go.Scatter(x=timepoints[::skip], y=ss_freq, name="freq_SimpleSim", legendgroup="ss",
                                     line=dict(width=2, color="silver")), secondary_y=True, row=2, col=1)
        ...
        if out_networkSim:
            ...
            fig.add_trace(go.Scatter(x=timepoints[::skip], y=ss_pow, name="power_NetworkSim", legendgroup="netsim",
                                     line=dict(width=4, color="lawngreen")), row=2, col=1)
            fig.add_trace(go.Scatter(x=timepoints[::skip], y=ss_freq, name="freq_NetworkSim", legendgroup="netsim",
                                     line=dict(width=2, color="mediumvioletred")), secondary_y=True, row=2, col=1)
        ...
        fig.update_layout(
            ...
            coloraxis1=dict(colorbar_title="dB", colorbar_x=0.97, colorbar_y=0.8, colorbar_len=0.4,
                            colorbar_thickness=10, colorscale="Viridis", cmin=0, cmax=cmax_pow),
            coloraxis2=dict(colorbar_title="Hz", colorbar_x=0.97, colorbar_y=0.2, colorbar_len=0.4,
                            colorbar_thickness=10, cmin=0, cmax=20),
            title="Calibration TransferOne (PP->NMM)  _" + mode, legend=dict(orientation="h"), template="plotly_white")

        pio.write_html(fig, file=folder + "/CALIB_TransferOne_" + mode + ".html", auto_open=auto_open)


    elif "vCC" in mode:

        _, ABt, _, TAUt, ABdam, TAUdam, He, Cee, Cie, POWdam = np.average(out_prop[1], axis=2).transpose()

        avg_weights = np.array([np.average(t_list[0]) for t_list in out_prop[2]])
        ...
        ## 1. HEATMAPS
        main_folder = "E:\LCCN_Local\PycharmProjects\\brainModels\FrequencyChart\\data\\"
        simulations_tag = "PSEmpi_FreqCharts3.0-m01d25y2023-t16h.38m.33s"  # Tag cluster job
        # mode = "classical" if mode=="classic" else mode
        df = pd.read_csv(main_folder + simulations_tag + "/FreqCharts_classical&fixed.csv")
        ...
        fig.add_trace(go.Heatmap(z=chart1_df.roi1_auc, x=chart1_df.He, y=chart1_df.Cie, coloraxis="coloraxis1"), row=1, col=2)
        fig.add_trace(go.Heatmap(z=chart1_df.roi1_Hz, x=chart1_df.He, y=chart1_df.Cie, coloraxis="coloraxis2"), row=2, col=2)

        # 1.2 Add trajectories and hovertexts He - Cie
        hovertext = ["   <b>t%0.2f</b><br>He = %0.2f  |  Cie = %0.2f<br>Power (dB) %0.4f"
                     % (timepoints[ii], He[ii], Cie[ii],
                        chart1_df["roi1_auc"].iloc[np.argsort(np.abs(chart1_df["He"] - He[ii]) + np.abs(chart1_df["Cie"] - Cie[ii])).values[0]])
                     for ii in range(len(He))]
        ...
        hovertext = ["   <b>t%0.2f</b><br>He = %0.2f  |  Cie = %0.2f<br>Frequency (dB) %0.4f"
                     % (timepoints[ii], He[ii], Cie[ii],
                        chart1_df["roi1_Hz"].iloc[np.argsort(np.abs(chart1_df["He"] - He[ii]) + np.abs(chart1_df["Cie"] - Cie[ii])).values[0]])
                     for ii in range(len(He))]
        fig.add_trace(go.Scatter(x=He, y=Cie, mode="lines+markers", showlegend=False, hoverinfo="text",
                                 hovertext=hovertext,
                                 line=dict(color=px.colors.sequential.YlOrBr[3], width=3), opacity=0.7), row=2, col=2)
        ...
        ### 2.1 Add heatmaps Cee-p
        chart2_df = df.loc[(df["mode"] == "classical" + "&fixed") & (df["exp"] == "exp_2")]
        fig.add_trace(go.Heatmap(z=chart2_df.roi1_auc, x=chart2_df.p, y=chart2_df.Cee, coloraxis="coloraxis1"), row=1, col=3)
        fig.add_trace(go.Heatmap(z=chart2_df.roi1_Hz, x=chart2_df.p, y=chart2_df.Cee, coloraxis="coloraxis2"), row=2, col=3)

        ## Add trajectories for Power and freque
        hovertext = ["   <b>t%0.2f</b><br>Cee = %0.2f  |  lrc/p = %0.2f<br>Power (dB) %0.4f"
                     % (timepoints[ii], Cee[ii], avg_weights[ii],
                        chart2_df["roi1_auc"].iloc[
        ...
        fig.add_trace(go.Scatter(x=[avg_weights[0]], y=[Cee[0]], mode="markers", showlegend=False, hoverinfo="text",
                                 hovertext=hovertext[0],
                                 line=dict(color="red", width=4)),
                      row=1, col=3)  # add initial point

        hovertext = ["   <b>t%0.2f</b><br>Cee = %0.2f  |  lrc/p = %0.2f<br>Frequency (Hz) %0.4f"
                     % (timepoints[ii], Cee[ii], avg_weights[ii],
        ...
        fig.update_layout(
            xaxis1=dict(title="Time (years)"), xaxis2=dict(title="He"), xaxis3=dict(title="p (input)"),
            xaxis4=dict(title="Time (years)"), xaxis5=dict(title="He"), xaxis6=dict(title="p (input)"),
            yaxis1=dict(title="Protein Concentration (M)"), yaxis2=dict(title="Parameter value"),
            yaxis3=dict(title="Cie"), yaxis4=dict(title="Cee"), yaxis5=dict(title="<b>Power (dB)"),
            yaxis6=dict(title="Frequency (Hz)", range=[0, 14]), yaxis7=dict(title="Cie"), yaxis8=dict(title="Cee"),
...
def propagationtrajectory_on4D(out_prop, mode, PSE3d_tag, folder="figures", auto_open=True):
    ...
    pio.write_html(fig, file=folder + "/Trajectory_onAnimatedPSE4D_"+mode+".html", auto_open=auto_open, auto_play=False)


def TransferTwo(out_circ, skip=1, mode="classic", folder="figures", auto_open=True):

    timepoints = out_circ[0]

    if "circ" in mode or "cModel" not in mode:
        if "circ" in mode:
            _, ABt, _, TAUt, ABdam, TAUdam, He, Hi, taue, taui, POWdam = np.average(out_circ[1], axis=2).transpose()

        elif "cModel" not in mode:
        ...
        pio.write_html(fig, file=folder + "/CALIB_TransferTwo_" + mode + ".html", auto_open=auto_open)

    elif "cModel" in mode:
        _, ABt, _, TAUt, ABdam, TAUdam, He, Cie, Cee, POWdam = np.average(out_circ[1], axis=2).transpose()

        fig = make_subplots(rows=2, cols=2,
                            specs=[[{"secondary_y": True}, {"rowspan": 2}], [{"secondary_y": True}, {}]],
                            subplot_titles=["Protein Propagation Model", "delta(Power)", "Simulated NMM dynamics", ""],
                            horizontal_spacing=0.25)
        ...
        # 2.2 NMM parameters
        for i, pair in enumerate([[He, "He"], [Cie, "Cie"], [Cee, "Cee"]]):
            trace, name = pair
            fig.add_trace(go.Scatter(x=timepoints, y=trace, name=name, legendgroup="nmm",
                                     line=dict(width=3, dash="dash", color=cmap_p[i])), secondary_y=True, row=1, col=1)
