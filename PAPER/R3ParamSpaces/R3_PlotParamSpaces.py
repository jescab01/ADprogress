...
# Define PSE folder
simulations_tag = "PSEmpi_ADpgCirc_vCC-m05d09y2023-t17h.10m.29s"  # Tag cluster job
...
fig.update_layout(yaxis1=dict(title="He<br>upper limit", title_standoff=0, tickvals=tickvals[0], ticktext=ticktext[0]),
                  yaxis5=dict(title="Cip<br>lower limit", title_standoff=0, tickvals=tickvals[1], ticktext=ticktext[1]),
                  yaxis9=dict(title="Cep<br>lower limit", title_standoff=0, tickvals=tickvals[2], ticktext=ticktext[2]),
...
fig.update_layout(yaxis1=dict(title="Cip<br>lower limit<br>(cABinh=0.4, cTAUinh=0)", title_standoff=0, tickvals=tickvals[1], ticktext=ticktext[1]),
                  yaxis5=dict(title="Cip<br>lower limit<br>(cABinh=0, cTAUinh=1.8)", title_standoff=0, tickvals=tickvals[1], ticktext=ticktext[1]),
                  height=500, width=1200)

pio.write_html(fig, file=main_folder + simulations_tag + "/PSE_ADpgCirc" + title + "_aux.html", auto_open=True)
pio.write_image(fig, file=main_folder + simulations_tag + "/PSE_ADpgCirc" + title + "_aux.svg")


## AUX plot: spatial dissociation with tau
...
pio.write_html(fig, file=main_folder + simulations_tag + "/PSE_ADpgCirc" + title + "_aux-AntPosCurves.html", auto_open=True)
pio.write_image(fig, file=main_folder + simulations_tag + "/PSE_ADpgCirc" + title + "_aux-AntPosCurves.svg")
