...
simulations_tag = "PSEmpi_ADpg-m08d08y2022-t19h.27m.41s"  # Tag cluster job
df = pd.read_csv(main_folder + simulations_tag + "/results.csv")

# Average out repetitions
df_avg = df.groupby(["subject", "model",  "g", "s", "band"]).mean().reset_index()
...
for j, cond in enumerate(["HC-fam", "HC", "FAM", "QSM", "MCI", "MCI-conv"]):
    ...
    subset = df_avg.loc[(df_avg["subject"] == cond) & (df_avg["band"] == "3-alpha")]
    ...
    fig.add_trace(
        go.Heatmap(z=subset.max_cx-subset.min_cx, x=subset.s, y=subset.g, colorscale='Viridis', showscale=False),
        row=2, col=(1 + j))
