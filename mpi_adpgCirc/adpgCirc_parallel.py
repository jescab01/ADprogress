...
def adpgCirc_parallel(params_):
    ...
    if "LCCN_Local" in os.getcwd():
        ...
        from ADprogress.ADpg_functions import CircularADpgModel_vH
    ...
    for ii, set in enumerate(params_):
        ...
        print(set)
        maxHe, mCie, mCee, maxTAU2SC, rho, HAdamrate, seedAB, seedTAU = set
        ...
        ABt_initMap = [0.005 / len(AB_seeds) if roi in AB_seeds else 0.005 / len(AB_seeds) * seedAB for roi in conn.region_labels]
        TAUt_initMap = [0.0005 / len(TAU_seeds) if roi in TAU_seeds else 0.0005 / len(TAU_seeds) * seedTAU for roi in conn.region_labels]
        ...
        bnm_simL, transient, bnm_dt = 10, 2, 2  # Units (seconds, seconds, years)

        circmodel = CircularADpgModel_vCC(
            conn, AB_initMap, TAU_initMap, ABt_initMap, TAUt_initMap, AB_initdam, TAU_initdam, HA_initdam,
            TAU_dam2SC=5e-2, maxTAU2SCdam=maxTAU2SC, HA_damrate=HAdamrate, #maxHAdam=maxHAdam,  # origins @ ¿?, 0.01, 1.5
        ...
        result.append(np.array([[maxHe]*len(peak_freqs), [mCie]*len(peak_freqs),
                                ...
                                [HAdamrate]*len(peak_freqs), [seedAB]*len(peak_freqs), [seedTAU]*len(peak_freqs),
                                timepoints, peak_freqs, pow_alpha,
