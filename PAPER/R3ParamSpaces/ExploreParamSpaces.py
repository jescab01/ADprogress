...
# Define PSE folder
simulations_tag = "PSEmpi_ADpgCirc_vCC-m05d08y2023-t10h.05m.29s"  # Tag cluster job
...
params = ["maxHe", "minCie", "minCee", "TAU2SC", "maxHAdam"]
...
results["maxTAU2SC_p"] = results.TAU2SC.values * 100

params = ["maxHe_p", "minCie_p", "minCee_p", "TAU2SC"]
...
with open(main_folder + simulations_tag + "\\fixed_params.txt", "w") as f:
    f.write("Init parameters:  He" + str(init_paramvalues[0]) + "; Cie" + str(init_paramvalues[1]) + "; minCee" + str(init_paramvalues[2]) + "; TAU2SC" + str(init_paramvalues[3]))
    f.write("Fixed change limits:  maxHe" + str(fixed_params[0]) + "; minCie" + str(fixed_params[1]) + "; minCee" + str(fixed_params[2]) + "; TAU2SC" + str(fixed_params[3]))
    f.close()
...
fig.update_layout(yaxis1=dict(title="He<br>upper limit", title_standoff=0, tickvals=tickvals[0], ticktext=ticktext[0]),
                  ...
                  yaxis13=dict(title="SC<br>damage", type="log"),  yaxis14=dict(type="log"), yaxis15=dict(type="log"), yaxis16=dict(type="log"),
...
# ## Dirty..
# params = [("TAU2SC", 0.3), ("TAU2SC", 0.5), ("TAU2SC", 0.8)]
# x_bar = [-0.045+(j*1.045/len(measures)+1/len(measures)) for j, measure in enumerate(measures)]
# x_bar = [0.195, 0.466, 0.735, 1.005]
# fig = make_subplots(rows=len(params), cols=len(measures), x_title="Time (years)",
#                     shared_xaxes=True, horizontal_spacing=0.075, shared_yaxes=True,
#                     column_titles=["Frequency peak", "Relative alpha power", "Firing rate", "Functional connectivity"])
#
# for i, (param, p2) in enumerate(params):
#
#     subset = results.loc[results["maxTAU2SC"]==p2].groupby([param, "time"]).mean().reset_index()
#     # subset = results.loc[results["minHi"] == minHi]
#     sl = True if i == 0 else False
#     for j, measure in enumerate(measures):
#         fig.add_trace(
...
#         # fig.add_hline(y=fixed_params[i], line=dict(width=6, color="red"), opacity=0.3, row=i+1, col=j+1)
#
# fig.update_layout(yaxis1=dict(title="TAU2SC<br>maxdam=0.3", title_standoff=0),
#                   yaxis5=dict(title="TAU2SC<br>maxdam=0.5", title_standoff=0),
#                   yaxis9=dict(title="TAU2SC<br>maxdam=0.8", title_standoff=0),
#                   height=750, width=1200)
#
